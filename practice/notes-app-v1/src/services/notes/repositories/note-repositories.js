...
class NoteRepositories {
  ...
  async createNote({ title, body, tags, owner }) {
    ...
    const query = {
      text: 'INSERT INTO notes(id, title, body, tags, owner, created_at, updated_at) VALUES($1, $2, $3, $4, $5, $6, $7) RETURNING id, title, body, tags, owner, created_at, updated_at',
  ...
  async getNotes(owner) {
    const result = await this.pool.query(`SELECT notes.* FROM notes
    LEFT JOIN collaborations ON collaborations.note_id = notes.id
    WHERE notes.owner = $1 OR collaborations.user_id = $1
    GROUP BY notes.id`, [owner]);
    return result.rows;
  }
